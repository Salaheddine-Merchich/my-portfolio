    ...
    <div className="py-20 px-6 max-w-7xl mx-auto" id="projects">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {bigProjects.projects.map((project, i) => (
          ...
          >
            {project.image && (
              <div className={`h-48 overflow-hidden relative group flex items-center justify-center p-8 transition-colors duration-300 ${
              ...
              {project.footerLink && (
                <div className="flex flex-wrap gap-3 mt-auto">
                  {project.footerLink.map((link, j) => (
                    ...
                    >
                      {link.name.toLowerCase().includes("github") ? <GitFork size={16} /> : <ExternalLink size={16} />}
                      {link.name}
                    </a>
                  ))}
                </div>
